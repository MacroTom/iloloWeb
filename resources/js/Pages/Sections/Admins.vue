...
export default{
    ...
    methods:{
        ...
        handleClick(evt){
            ...
            if(target.classList.contains('p-link') || target.nodeName === 'svg' || target.nodeName === 'path'){
                const page = this.current_page === 0 ? 1 : Math.floor((this.current_page/this.admins?.per_page))+1;
                this.gotoPage(page);
            }
        },
        searchUser(){
            router.get(`admins?q=${this.search}`, {},
            {
                preserveState: true,
                onSuccess: (res) => {
                    // this.$toast.add({ severity: 'success', summary: 'Successful', detail: 'Data Retrieved!', life: 3000 });
    ...
    mounted(){
        var page = this.admins?.current_page === 1 ? this.admins?.current_page :
        this.admins?.per_page * (this.admins?.current_page - 1);
        ...
        <div class="col-12">
            <div class="p-4 card lg:p-5">
                ...
                <DataTable
                    ...
                    :value="admins?.data ? admins?.data : admins"
